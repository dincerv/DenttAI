...
@router.get("/health", summary="Auth servis sağlık kontrolü (CSRF token için)")
async def auth_health() -> dict:
    """Frontend ensureCsrfToken bu endpoint'i çağırır; 404 olursa login kırılır.

    Gerçek X-CSRF-Token header'ını CSRFMiddleware GET yanıtına ekler.
    """
    return {"status": "ok", "service": "auth", "csrf": "middleware"}
