...
router = APIRouter(prefix="/integration/whatsapp", tags=["whatsapp-settings"])
...
async def get_doctor_settings(
    ...
    clinic_id = current_user["clinic_id"]
    doctor_id = current_user.get("doctor_id") or current_user["user_id"]
...
async def create_faq(
    ...
    faq = await service.create(clinic_id, data, current_user["user_id"])
    await db.commit()
    return faq
