...
async def process_incoming_whatsapp_message(
    ...
    """
    Webhook/ingest gelen mesajı işler.
    Mevcut whatsapp_tasks._process_message_impl üzerine ince sarmalayıcı.
    """
    from app.tasks.whatsapp_tasks import _process_message_impl

    ts = int(timestamp) if timestamp not in (None, "") else 0
    await _process_message_impl(
