...
async def register_clinic(
    ...
    try:
        validated_email = validate_email(data.admin_email.strip(), check_deliverability=False).normalized
...
async def login(data: LoginRequest, db: AsyncSession) -> TokenResponse:
    ...
    try:
        validated_email = validate_email(data.email.strip(), check_deliverability=False).normalized
    except EmailNotValidError as e:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail="Geçersiz e-posta adresi formatı",
